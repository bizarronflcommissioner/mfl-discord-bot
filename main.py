...
async def load_franchises():
    url = f"https://www.myfantasyleague.com/{SEASON_YEAR}/export?TYPE=league&L={LEAGUE_ID}&JSON=1"
...
async def load_players():
    url = f"https://www.myfantasyleague.com/{SEASON_YEAR}/export?TYPE=players&L={LEAGUE_ID}&JSON=1"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            data = await resp.json()
...
async def fetch_all_transactions():
    url = f"https://www.myfantasyleague.com/{SEASON_YEAR}/export?TYPE=transactions&L={LEAGUE_ID}&TRANS_TYPE=ALL"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            if resp.status != 200:
...
async def fetch_draft():
    url = f"https://www.myfantasyleague.com/{SEASON_YEAR}/export?TYPE=draftResults&L={LEAGUE_ID}&JSON=1"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            if resp.status != 200:
...
async def draft_loop():
    ...
    while not client.is_closed():
        ...
        for i, pick in enumerate(picks):
            pick_id = pick.get("timestamp")
            if not pick_id or pick_id in posted_picks:
                continue
